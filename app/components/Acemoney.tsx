...
const AceMoneyTab = () => {
  ...
  console.log("forasfkfjbwdovjb", posts);
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-neutral-200 font-sans antialiased selection:bg-red-500/30">
      <main className="w-full max-w-[935px] mx-auto pt-10 md:pt-16 pb-20 px-4 md:px-0">
        ...
        <Link href={"/project"}>
          {" "}
          <span className="text-white text-xl underline cursor-pointer">
            Back
          </span>
        </Link>
        <motion.header
        ...
        >
          ...
          <div className="flex flex-col items-center md:items-start">
            <h1 className="text-3xl md:text-4xl font-black text-white tracking-[0.25em] cursor-default uppercase">
              {projectName}
            </h1>
          </div>
        </motion.header>
          ...
          <div className="mt-8 md:mt-12 min-h-[500px]">
            <AnimatePresence mode="wait">
              ...
              {activeTab === "thumbnails" && (
                ...
                >
                  {imagePosts.length === 0 ? (
                    <div className="col-span-3 py-24 text-center text-neutral-600 tracking-widest text-sm uppercase font-black">
                      No thumbnails yet.
                    </div>
                  ) : (
                    imagePosts.map((post) => (
                      <motion.div
                        key={post.id}
                        whileHover={{ y: -5 }}
                        className="aspect-[9/16] bg-[#111] relative group cursor-pointer overflow-hidden rounded-[24px] border border-white/5"
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                          <span className="text-[10px] font-black uppercase tracking-widest text-red-500 mb-1">
                            click to see
                          </span>
                        </div>
                      </motion.div>
